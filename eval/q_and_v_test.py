...
def test(model, data_loader, loss_fn, num_classes, feature_extractor, obj_detector):
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader, 0):
            ...
            v_inputs, q_inputs, q_lens, ys, q_ids =\
            ...
            if args.model != 'mac':
                model.init_hidden()
            output = model(v_inputs, q_inputs, v_lens, q_lens)[:num_real_examples]
...
if __name__=='__main__':
    ...
    # Save test results (targets, predictions, question types)
    t, p, q = test(model, test_loader, loss_fn, num_classes, feature_extractor, obj_detector)
