...
from models.v_only_cnn3d import VideoOnlyCNN3D
...
def test(model, data_loader, loss_fn, num_classes):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader, 0):
            ...
            y_target = np.append(y_target, ys)
            ...
            # CUDA inputs
            if use_cuda:
                v_inputs, ys = v_inputs.cuda(), ys.cuda()

            output = model(v_inputs)[:num_real_examples]
...
if __name__=='__main__':
    ...
    test_data = VNQADataset(q_dir=QUESTIONS_DIR, v_dir=VIDEOS_DIR, v_only=True,
                            num_classes=num_classes, filenames=test_file_ids, labels=labels)
